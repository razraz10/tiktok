import { CommentsCompType } from "@/app/types";
...
export default function Comments({ params }: CommentsCompType) {
...
const addComment = async ()=>{
  ...
  return (
    <>
      ...
      >
        ...
        <ClientOnly>
          {commentsByPost.length < 1 ? (
            <div className="text-center mt-6 text-xl text-gray-500">....עדיין אין תגובות </div>
          ) : (
            <div>
      ...
      className="absolute flex items-center justify-between bottom-0 bg-white h-[85px] w-full py-5 px-8 border-t-2 ">
        ...
        `}>
          ...
          <input 
          className="bg-[#F1F1F2] text-[14px] focus:outline-none w-full lg:max-w-[420px] p-2 rounded-full "
