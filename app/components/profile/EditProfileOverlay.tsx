...
export default function EditProfileOverlay() {
  ...
  const validate = () => {
    ...
    if (!userName) {
      setError({type: 'userName', message: 'חסר שם משתמש'})
      isError = true
    }
    return isError
  }
  ...
  const updateUserInfo = async ()=>{
    ...
    if(isError) return
    if(!contextUse?.user) return

    try {
      setIsUpdating(true)
      await useUpdateProfile(currentProfile?.id || '', userName, userBio)
      setCurrentProfile(contextUse?.user?.id)
      ...
      >
        ...
        >
          <div className="absolute flex items-center justify-between w-full p-5 left-0 top-0 border-b border-b-gray-300  ">
            <h1 className="text-[22px] font-medium">ערוך פרופיל</h1>
            <button
              disabled={isUpdating}
            ...
            {!uploadedImage ? (
              <div>
                ...
                >
                  ...
                  <div className="flex items-center justify-center sm:-mt-6">
                    <div className="sm:w-[60%] w-full max-w-mb">
                        <TextInput 
                        string={userName}
                        ...
                        <p className={`relative text-[11px] text-gray-500 ${error ? 'mt-1' :  'mt-4'} `}>
                                שם משתמש יכול להיות רק אותיות, מספרים, ונקודות.
                                 שינוי של השם משתמש ישנה גם את הקישור לפרופיל שלך
                        </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="w-full max-h-[420px] mx-auto bg-black circle-stencil ">
                ...
                onChange={(cropper: CropperRef) => setCropper(cropper.getCoordinates())}
                src={uploadedImage}
                />
              </div>
            )}
                ...
                {!uploadedImage ? (
                    <div id="UpdateInfoButtons" className="flex items-center justify-end">
                        ...
                        className="flex items-center border rounded-sm px-3 py-[6px] hover:bg-gray-100 ">
                            <span className="px-2 font-medium text-[15px]">בטל</span>
                        </button>
                        <button 
                        disabled={isUpdating}
                        ...
                        className="flex items-center bg-[#F02C56] text-white border rounded-md ml-3 px-3 py-[6px] ">
                            <span className="px-2 font-medium text-[15px]">
                                {isUpdating ? (<BiLoaderCircle color="#ffffff" className='my-1 mx-2.5 animate-spin' />) : 'שמור'}
                            </span>
                        </button>
                    </div>
                ) : (
                    <div id="CropperButtons" className="flex items-center justify-end">
                        ...
                        className="flex items-center bg-[#F02C56] text-white border rounded-md ml-3 px-3 py-[6px] ">
                            <span className="px-2 font-medium text-[15px]">
                                {isUpdating ? (<BiLoaderCircle color="#ffffff" className='my-1 mx-2.5 animate-spin' />) : 'החל'}
                            </span>
                        </button>
                    </div>
                )}
